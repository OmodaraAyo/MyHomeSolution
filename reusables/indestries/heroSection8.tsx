...
import { HeroSection8Props } from "../types/types";

export default function HeroSection8(props: HeroSection8Props) {
    ...
    >
      ...
      >
        <div className="flex flex-col items-start gap-1 w-full">
          ...
          <p className={`text-[1.6rem] md:text-5xl font-medium lg:font-bold leading-11 md:leading-20 w-full max-w-80 sm:max-w-xl lg:max-w-3xl ${sora.className}`}>{subTitle}</p>
          <p className="max-w-[43.2rem] text-[#FFFFFF] text-xs lg:text-lg">{description}</p>
        </div>
      </motion.div>
    </motion.section>
  );
